## "run_analysis_v2.R" is identical with "run_analysis.R" script with only 
## one exception: it's using the LaF package for reading the train and test
## data sets.
##
## Requirements:
...
data_filter_v2 <- function(dir_name = "UCI HAR Dataset") {
    ## Get the date and time of processing as a string "YYYY-MM-DD_HH-MM-SS"
    d <- gsub(" ", "_", fixed = T, 
              gsub(":", "-", fixed = T, as.character(Sys.time())))
    
    ## Check if we have available the 'data.table' package and load it
    if ('LaF' %in% rownames(installed.packages())) {
        invisible(library(LaF))
    } else {
        message(paste("The script requires the 'LaF' package which",
                      "is not currently installed. Install the 'LaF'",
                      "package and then source the run_analysys_v2.R:\n",
    ...
    ## using the fixed file format read function provided by the LaF package
    d.train <- laf_open_fwf(filename=data_files["X_train"], 
                            column_types=rep("numeric", 561), 
                            column_widths=rep(16, 561))
    d.test <- laf_open_fwf(filename=data_files["X_test"], 
                           column_types=rep("numeric", 561), 
                           column_widths=rep(16, 561))
    ml.data <- as.data.frame(rbind(d.train[,], d.test[,]))
    names(ml.data) <- feature_labels[,2]
